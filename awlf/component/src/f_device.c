#include "../include/f_device.h"
#include "../include/f_mem.h"

static LIST_HEAD(dev_list);

inline static uint8_t IS_DEV_OTYPE_NOT_SET(f_device_t dev, otype_e otype)
{
    return (otype & (~dev->__priv.otype));
}

inline uint32_t DEV_GET_REGFLAGS(f_device_t dev)
{
    return dev->__priv.reg_falgs;
}

inline uint32_t DEV_GET_OTYPE(f_device_t dev)
{
    return dev->__priv.otype;
}

inline void SET_DEV_STATUS(f_device_t dev, uint32_t _status)
{
    __SET_FLAG(dev->__priv.status, (_status));
}

inline void CLR_DEV_STATUS(f_device_t dev, uint32_t _status)
{
    __CLR_FLAG(dev->__priv.status, (_status));
}

inline uint32_t IS_DEV_STATUS_SET(f_device_t dev, uint32_t _status)
{
...
awlf_ret_t device_register(f_device_t dev, char* name, uint32_t reg_flags)
{
    uint32_t    primask;
