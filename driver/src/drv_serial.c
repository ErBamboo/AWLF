...
/* 无FIFO情况下的 TX 函数*/
static size_t _serial_tx_poll(f_device_t dev, void* pos, void *data, size_t len)
{
    size_t length;
    drv_serial_t serial = (drv_serial_t)dev;
...
static size_t _serial_rx_poll(f_device_t dev, void* pos, uint8_t *buf, size_t len)
{
    size_t length;
    drv_serial_t serial = (drv_serial_t)dev;
 ...
 */
static size_t _serial_tx_block(f_device_t dev, void* pos, void *data, size_t len)
{
    drv_serial_t   serial;
    serial_fifo_t  tx_fifo;
    size_t         sended = 0;
    serial = (drv_serial_t)dev;
    ...
    do
    {
        ...
        if (tx_fifo->load_size > 0) {
            ...
            f_completion_wait(&tx_fifo->cpt, WAIT_FOREVER);
        }
    } while (len); // 当发送完成后退出
    tx_fifo->load_size = 0;
    CLR_DEV_STATUS(dev, DEV_STATUS_BUSY_TX);
...
/* 串口非阻塞发送 */
static size_t _serial_tx_nonblock(f_device_t dev, void* pos, void *data, size_t len)
{
    drv_serial_t   serial;
    serial_fifo_t  tx_fifo;
    uint32_t       primask;
...
static awlf_ret_t _serial_tx_enable(f_device_t dev)
{
    drv_serial_t serial = (drv_serial_t)dev;
    ...
    if(!tx_fifo)
    {
        ...
        */
        ...
        tx_fifo = (serial_fifo_t)f_alloc(sizeof(serial_fifo_s));
    ...
    if(DEV_GET_OTYPE(dev) & OTYPE_BLOCKING_TX)
        f_completion_init(&tx_fifo->cpt, CompEvent_SerialTx);
...
static awlf_ret_t _serial_rx_enable(f_device_t dev)
{
    drv_serial_t serial;
    ...
    if(!rx_fifo)
    {
        ...
        rx_fifo = (serial_fifo_t)f_alloc(sizeof(serial_fifo_s));
    ...
    if(DEV_GET_OTYPE(dev) & OTYPE_BLOCKING_TX)
        f_completion_init(&rx_fifo->cpt, CompEvent_SerialRx);
...
/************************** IO API ***************************************************************/
static awlf_ret_t serial_init(f_device_t dev)
{
    drv_serial_t serial = (drv_serial_t)dev;
...
static awlf_ret_t serial_open(f_device_t dev, otype_e otype)
{
    drv_serial_t serial = (drv_serial_t)dev;
...
static awlf_ret_t serial_ctrl(f_device_t dev, size_t cmd, void* arg)
{
    drv_serial_t serial = (drv_serial_t)dev;
    if(!serial || !serial->interface || !serial->interface->control)
        return AWLF_ERROR_PARAM;
...
static size_t serial_read(f_device_t dev, void* pos, void* buf, size_t len)
{
    drv_serial_t serial;
    serial_fifo_t rx_fifo;
    size_t recv_len = 0;
        ...
        while(recv_len < len)
        {
            ...
            if(remaining_size)
            {
                rx_fifo->load_size = (remaining_size > ringbuf_cap(&rx_fifo->rb)) ?
                                     ...
                                     remaining_size;
                f_completion_wait(&rx_fifo->cpt, WAIT_FOREVER);
    ...
    primsk = f_hw_disable_irq();
    recv_len = ringbuf_out(&rx_fifo->rb, buf, len);
    f_hw_enable_irq(primsk);
    return recv_len;
}
 ...
 */
 inline uint32_t serial_get_rxmask(drv_serial_t serial)
{
    return (((uint32_t)(serial->cfg.parity == PARITY_NONE) << ((serial->cfg.databits >> 16))) | serial->cfg.databits) & 0x1FF;
}

awlf_ret_t serial_register(drv_serial_t serial, char* name, void* handle, uint32_t regflag)
{
...
awlf_ret_t serial_hw_isr(drv_serial_t serial, serial_event_e event, void* arg, size_t arg_size)
{
    if(!serial || !serial->interface || !serial->interface->control)
        return AWLF_ERROR_PARAM;
...
awlf_ret_t serial_init_fifo(drv_serial_t serial, serial_fifo_t fifo, uint8_t* buf, size_t bufsz)
{
...
inline serial_fifo_t serial_get_rxfifo(drv_serial_t serial)
{
    return serial->__priv.rx_fifo;
}

/* 从串口FIFO中读取数据 */
static size_t serial_get_fifo_data(drv_serial_t serial, uint8_t *buf, size_t len)
{
    serial_fifo_t  fifo;
